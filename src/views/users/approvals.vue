<template>
  <div class="usersApprovalsContainer">
      ...
      <div class="topTitle">
        <div>
          ...
          <el-radio-group v-model="approvalsTypes" style="margin:5px 0">
            <el-radio v-for="item in approvalsType" :key="item.deploymentId" :label="item.key" :value="item.key" @change="changeSelectParams">{{ item.name }}</el-radio>
          </el-radio-group>
        </div>
        <div v-if="tagName != 'approvals'">
          <span>审批状态：</span>
          <el-checkbox-group v-model="approvalsStates">
            <el-checkbox v-for="item in approvalsState" :key="item.id" :label="item.id" :value="item.id" @change="changeSelectParams">{{ item.value }}</el-checkbox>
          </el-checkbox-group>
        </div>
      </div>
      <div>
        ...
        <el-table :data="tableData" style="width: 100%" :default-sort="{ order: 'descending', prop: 'procApplyTime' }">
          ...
          <el-table-column label="审批状态">
            <template slot-scope="scope">
              <span v-if="scope.row.processState === '0'" class="rovalsState"> <em class="sub">已提交</em> </span>
              <span v-if="scope.row.processState === '1'" class="rovalsState"> <em class="stay">审批中</em> </span>
              <span v-if="scope.row.processState === '2'" class="rovalsState"> <em class="adopt">审批通过</em> </span>
              <span v-if="scope.row.processState === '3'" class="rovalsState"> <em class="reject">审批不通过</em> </span>
              <span v-if="scope.row.processState === '4'" class="rovalsState"> <em class="revoke">撤销</em> </span>
            </template>
          </el-table-column>
          <el-table-column label="操作">
            <template slot-scope="scope">
              <!--  && (item.row.stateOfApproval == '待审批' || item.row.stateOfApproval == '已驳回') -->
              <el-button v-show="tagName == 'launch' && scope.row.processState === '1'" size="mini" type="text" @click="clickPass('4', scope.row.processId)">撤销</el-button>
              <!--  && item.row.currentApproverId == userId -->
              <el-button v-show="(tagName == 'copy' || tagName == 'approvals') && scope.row.processState === '1'" size="mini" type="text" @click="clickPass('2', scope.row.processId)">通过</el-button>
              <!--  && item.row.currentApproverId == userId -->
              <el-button v-show="(tagName == 'copy' || tagName == 'approvals') && scope.row.processState === '1'" size="mini" type="text" @click="clickPass('3', scope.row.processId)">驳回</el-button>
              <el-button size="mini" type="text" @click="clickDetail(scope.row.processId, scope.row.processName)">查看</el-button>
              <!-- <el-button size="mini" type="danger">打印</el-button> -->
            </template>
          </el-table-column>
        </el-table>
        <el-row type="flex" justify="center" style="height:60px" align="middle">
          <el-pagination :total="Number(total)" :page-sizes="[10, 20, 30, 50]" layout="prev, pager, next" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
        </el-row>
      </div>
    </div>
    <!--查看弹框-->
    <el-dialog :title="topLabel" :visible.sync="centerDialogVisible" width="50%" left>
      ...
      <Quit v-show="seeState == 'quit'" ref="quit" :selected-id="selectedId" :tab-lab="tagName" @closeDialog="closeDialog" />
      <!-- <Examine v-show="seeState == 'examine'" /> -->
      <Leave v-show="seeState == 'leave'" ref="leave" :selected-id="selectedId" :tab-lab="tagName" @closeDialog="closeDialog" />
      <Overtime v-show="seeState == 'overtime'" ref="overtime" :selected-id="selectedId" :tab-lab="tagName" @closeDialog="closeDialog" />
