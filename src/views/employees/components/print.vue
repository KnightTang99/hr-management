<template>
  <div id="Print" class="dashboard-container">
    <div class="app-container">
      <el-card id="print">
        ...
        <el-row type="flex" justify="end">
          <el-button v-print="printId" size="small" type="primary">打印</el-button>
        </el-row>
...
<script>
import { getPersonalById, getJobInfo, getBaseInfoById } from '@/api/employees'
export default {
  name: 'Print',
  data() {
    return {
  ...
  // 创建完毕状态
  created() {
    this.type === 'personal' ? this.getPersonalById() : this.getJobInfo()
  },
  // 组件更新
  methods: {
    async getPersonalById() {
      this.formData = await getPersonalById(this.userId) // 获取个人基本信息
    },
    async getJobInfo() {
      const userInfo = await getBaseInfoById(this.userId)
      const jobInfo = await getJobInfo(this.userId) // 获取个人基本信息
