    ...
    <el-form label-width="220px" :model="userInfo">
      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <image-upload ref="employeeAvatar" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
      <el-row class="inline-info" type="flex" justify="center">
        <el-col :span="12">
          ...
          <el-button type="primary" @click="saveBaseInfoById">保存更新</el-button>
        </el-col>
      </el-row>
    </el-form>
    <!-- 基础信息 -->
    <el-form label-width="220px" :model="formData">
      <div class="block">
        ...
        <el-form-item label="最高学历">
          <el-select v-model="formData.theHighestDegreeOfEducation" class="inputW2" placeholder="请选择">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <image-upload ref="employeePicture" :show-upload-btn="showUploadBtn" />
        </el-form-item>
        <el-form-item label="国家/地区">
          <el-select v-model="formData.nationalArea" class="inputW2" placeholder="请选择">
        ...
        <el-form-item label="婚姻状况">
          <el-select v-model="formData.maritalStatus" class="inputW2" placeholder="请选择">
        ...
        <el-form-item label="星座">
          <el-select v-model="formData.constellation" class="inputW2" placeholder="请选择">
            <el-option v-for="item in EmployeeEnum.constellation" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="血型">
          <el-select v-model="formData.bloodType" class="inputW2" placeholder="请选择">
      ...
      <div class="block">
        ...
        <el-row class="inline-info" type="flex" justify="center">
          <el-col :span="12">
            ...
            <el-button type="primary" @click="savePersonalById">保存更新</el-button>
          </el-col>
        </el-row>
      </div>
    </el-form>
...
import { getBaseInfoById, saveBaseInfoById, getPersonalById, savePersonalById } from '@/api/employees'
export default {
  name: 'PersonalInfo',
  data() {
    return {
  ...
  methods: {
    ...
    async getBaseInfoById() {
      this.userInfo = await getBaseInfoById(this.userId)
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        // 去除空格是因为可能没有图片，而接口要求传一个空字符串
        this.$refs.employeeAvatar.fileList = [{ url: this.userInfo.staffPhoto, upload: true }] // 直接赋值是因为只上传一张图片，fileList里面只会有一条数据，直接覆盖就行
      }
    },
    // 保存用户基础信息
    async saveBaseInfoById() {
      try {
        const fileList = this.$refs.employeeAvatar.fileList
        if (fileList.some(item => !item.upload)) {
          // upload作为是否上传成功的标志，有则可以取值并提交
          this.$message.warning('您当前还有图片未上传完成')
          return
        }
        await this.$confirm('确认更新信息吗')
        await saveBaseInfoById({ ...this.userInfo, staffPhoto: fileList && fileList.length ? fileList[0].url : ' ' }) // 这里判断条件是因为既要保证fileList里面有值，又不能是空值
        this.$message.success('更新成功')
      } catch (err) {
        this.$message('用户已取消更新')
      }
    },
    // 获取员工个人信息
    async getPersonalById() {
      this.formData = await getPersonalById(this.userId)
      if (this.formData.staffPhoto && this.formData.staffPhoto.trim()) {
        this.$refs.employeePicture.fileList = [{ url: this.formData.staffPhoto, upload: true }]
      }
    },
    // 保存员工个人信息
    async savePersonalById() {
      try {
        const fileList = this.$refs.employeePicture.fileList
        if (fileList.some(item => !item.upload)) {
          this.$message.warning('您当前还有图片未上传完成')
          return
        }
        await this.$confirm('确认更新个人信息吗')
        await savePersonalById({ ...this.formData, staffPhoto: fileList && fileList.length ? fileList[0].url : ' ' }) // 这里取第一个是因为这个组件需求就是只有一张照片
        this.$message.success('更新成功')
        this.$router.back()
      } catch (err) {
        this.$message('用户已取消更新')
